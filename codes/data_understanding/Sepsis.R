...
dat2 <- subset(dat, select = c(1:9,14))
summary(dat2)
...
barplot(prop.table(table(allData$SepsisLabel)), 
        col = rainbow(5), ylim = c(0,0.7), main = "Class distibution") #only 1.74% of septic
...
########### delete all NA's
l <- na.omit(allLab) 
summary(l) #only 180obs of 5 variables, needs to deal with missing values!
...
allab0 <- bind_cols(al0[,-10],l0) #???cannot just merge lab data and vital sign data from different groups
...
#create_report(datall)
##################### cut data into train and test 70/30%

#barplot(prop.table(table(datall$SepsisLabel)), 
        #col = rainbow(5), ylim = c(0,0.9), main = "Class distibution")
#plot the density  of labels vary from index
ggplot(train, aes(SepsisLabel)) +
  geom_density()
